...
import 'package:vidhaan_school_app/accounts/account_page.dart';
import 'package:http/http.dart' as http;
import 'package:vidhaan_school_app/attendance/Today%20Presents_Page.dart';
import 'package:vidhaan_school_app/exam/exam.dart';
import 'package:vidhaan_school_app/feedback_history/feedback%20history.dart';
import 'package:vidhaan_school_app/notification_controllers/Notifications.dart';
import 'package:vidhaan_school_app/profile/Profileview.dart';
import 'package:vidhaan_school_app/root_pages/Root_Page4.dart';
import 'package:vidhaan_school_app/root_pages/Root_page.dart';
...
class _FrontpageState extends State<Frontpage> with SingleTickerProviderStateMixin {
  ...
  PageController _pageController = PageController(); // Create a PageController object
// Initialize currentIndex to specify the initial index
  int currentIndex = 1;
  showwwaring() {
  ...
  Widget build(BuildContext context) {
    ...
    // Inside the onTap method of the button you want to trigger the navigation
/*    _pageController.animateToPage(
      1, // Index of the second page (indexing starts from 0)
  ...
  /*    appBar: AppBar(
        elevation: 50,
          actions: [
            IconButton(
              icon: Icon(Icons.menu, color: Color(0xff0069bd),), // Assuming this is your hamburger icon
              onPressed: null, // Disables onTap functionality
            ),
          ],
          iconTheme: IconThemeData(color:  Color(0xff0069bd),),
      // centerTitle: true,
      //     backgroundColor: Color(0xff0873C4),
          backgroundColor: Color(0xff0069bd),
          // backgroundColor: Colors.black,
       title: StreamBuilder(stream: constants.firestore2db!.collection('Admin').snapshots(), builder: (context, snapshot) {
          if(snapshot.hasData){
            return  Padding(
                    ...
                    children: [
                      Container(
                        ...
                        child: ClipRRect(
                          borderRadius: BorderRadius.circular(100),
                          child: CachedNetworkImage(
                            imageUrl:snapshot.data!.docs[0]['logo']
                            ,
                          ),
                        ),
                      ),
                      Container(
                        ...
                        child: Text(
                          // "Hi ${staffname}",
                          snapshot.data!.docs[0]['schoolname'],
                          maxLines: 2,
                          style: GoogleFonts.poppins(
                              color: Colors.white,
                              fontWeight: FontWeight.w600,
                              fontSize: width / 23.68),
                  ...
                  children: [
                    page == "Attendance" || page == "Home Works"
                      ...
                      children: [
                        ...
                        Padding(
                          padding: EdgeInsets.only(top: height/15.70833),
                          child:
                            FutureBuilder(future: constants.firestore2db!.collection('Admin').get(), builder: (context, snapshot) {
                              if(snapshot.hasData){
                                return InkWell(
                                  onTap : (){
                                    print(width);
                                  ...
                                  child: Container(
                                    height: height/13.7090909,
                                    width: width/1.2857142,
                                    decoration: BoxDecoration(
                                        color: Colors.white,
                                    ...
                                    child: Row(
                                      ...
                                      children: [
                                        Padding(
                                          padding: EdgeInsets.only(left: width/22.5, right: width/72),
                                          child: Container(
                                            width: width/1.8,
                                            child: Text(
                                              // "Hi ${staffname}",
                                    ...
                                    child: Row(
                                      ...
                                      children: [
                                        ...
                                        InkWell(
                                            ...
                                            child: Icon(
                                                _pageController == 0 ?
                                                Icons.chevron_right : Icons.chevron_right, color:  Color(0xff0873C4)))
                                      ],
                                    ),
                                  ),
                                ),
    ...
    if (document2!.docs.length > 0) {
      ...
      for (int i = 0; i < document!.docs.length; i++) {
        if (document!.docs[i]["admitclass"] == _typeAheadControllerclass.text &&
          ...
          if (present[i] == true) {
            sendPushMessage(document.docs[i]["token"],
