...
import 'package:vidhaan_school_app/face_dector/faceidstaff.dart';
...
/*  CollectionReference schoolLocation = constants.firestore2db!.collection(
      'SchoolLocation');*/
  ...
  barcodeScan() async {
    ...
    startBarcodeScanStream() async {
      FlutterBarcodeScanner.getBarcodeStreamReceiver(
          '#ff6666', 'Cancel', true, ScanMode.BARCODE)!.listen((barcode) =>
          print(barcode));
    }
  }
        ...
        child: AlertDialog(
          content: Column(
            children: [
              ...
              Text("Please Make Sure Valid Location Point",
              ...
              SizedBox(
                ...
                width: width / 3.60,
                child: Center(
                    child: Lottie.asset("assets/animation_lk2m7oge.json")),
              ),
          ...
          content: Column(
            children: [
              ...
              SizedBox(
                ...
                child: Center(
                  child: Lottie.asset(
                      "assets/animation_lk3vmbvu.json", fit: BoxFit.cover),
                ),
              ),
              SizedBox(height: height / 32.85,),
              GestureDetector(
  ...
  Marktheattendancefun() {
    ...
    constants.firestore2db?.collection("Staff_attendance").doc("${DateTime.now().day}${DateTime.now().month}${DateTime.now().year}").set({
  ...
  Marktheattendancefun2() async {
    var document = await constants.firestore2db?.collection("Staffs").doc(
        staffid).collection("Attendance").where("Date", isEqualTo: "${DateTime
        .now()
        .day}/${DateTime
        .now()
        .month}/${DateTime
  ...
  longDistance() {
    ...
    return AwesomeDialog(
        ...
        dialogType: DialogType.warning,
        animType: AnimType.rightSlide,
        title: "You are currently outside the school premises",
        desc: "Please come to school to mark your attendance.",
        btnOkOnPress: () {},
        btnOkText: "Ok"
    )
      ..show();
  ...
  getstaffdetails() async {
    ...
    for (int i = 0; i < document!.docs.length; i++) {
      ...
      if (staffid.isNotEmpty) {
        ...
        var document2 = await constants.firestore2db?.collection("Staffs").doc(
            staffid).
        collection("Attendance").where("Date", isEqualTo: "${DateTime
            .now()
            .day}/${DateTime
            .now()
            .month}/${DateTime
        ...
        if (document2!.docs.length > 0) {
          ...
          var document3 = await constants.firestore2db?.collection("Staffs")
              .doc(staffid).
          collection("Attendance").doc(document2!.docs[0].id)
              .get();
  ...
  verfiy(val) async {
    ...
    final faceData = faces.map((face) {
      ...
      return 'Face id: ${leftEyePosition!.x},${rightEyePosition!
          .x},${bottommouthPosition!.x},${rightmouthPosition!
          .x},${leftmouthPosition!.x},${nousePosition!.x},';
    }).join('\n');
    final faceData2 = faces.map((face) async {
      ...
      final nouseLandmark = face.landmarks[FaceLandmarkType.noseBase];
      final nousePosition = nouseLandmark?.position;
      var documet = await constants.firestore2db?.collection("Staffs").doc(
          staffid).collection("Facedate").doc(staffid).get();
      Map<String, dynamic>? value = documet!.data();
      int i = 0;
      setState(() {
        i = 0;
      });
      if (value!["leftEyePositionx"] <= leftEyePosition!.x + 250 &&
          value!["leftEyePositionx"] >= leftEyePosition!.x - 250) {
        setState(() {
          i = i + 1;
        });
      }
      if (value!["rightEyePositionx"] <= rightEyePosition!.x + 250 &&
          value!["rightEyePositionx"] >= rightEyePosition!.x - 250) {
        setState(() {
          i = i + 1;
        });
      }
      if (value!["bottommouthPositionx"] <= bottommouthPosition!.x + 250 &&
          value!["bottommouthPositionx"] >= bottommouthPosition!.x - 250) {
        setState(() {
          i = i + 1;
        });
      }
      if (value!["rightmouthPositionx"] <= rightmouthPosition!.x + 250 &&
          value!["rightmouthPositionx"] >= rightmouthPosition!.x - 250) {
        setState(() {
          i = i + 1;
        });
      }
      if (value!["leftmouthPositionx"] <= leftmouthPosition!.x + 250 &&
          value!["leftmouthPositionx"] >= leftmouthPosition!.x - 250) {
        setState(() {
          i = i + 1;
        });
      }
      if (value!["nousePositionx"] <= nousePosition!.x + 250 &&
          value!["nousePositionx"] >= nousePosition!.x - 250) {
        setState(() {
          i = i + 1;
        });
      }

      if (value!["leftEyePositiony"] <= leftEyePosition!.y + 250 &&
          value!["leftEyePositiony"] >= leftEyePosition!.y - 250) {
        setState(() {
          i = i + 1;
        });
      }
      if (value!["rightEyePositiony"] <= rightEyePosition!.y + 250 &&
          value!["rightEyePositiony"] >= rightEyePosition!.y - 250) {
        setState(() {
          i = i + 1;
        });
      }
      if (value!["bottommouthPositiony"] <= bottommouthPosition!.y + 250 &&
          value!["bottommouthPositiony"] >= bottommouthPosition!.y - 250) {
        setState(() {
          i = i + 1;
        });
      }
      if (value!["rightmouthPositiony"] <= rightmouthPosition!.y + 250 &&
          value!["rightmouthPositiony"] >= rightmouthPosition!.y - 250) {
        setState(() {
          i = i + 1;
        });
      }
      if (value!["leftmouthPositiony"] <= leftmouthPosition!.y + 250 &&
          value!["leftmouthPositiony"] >= leftmouthPosition!.y - 250) {
        setState(() {
          i = i + 1;
        });
      }
      if (value!["nousePositiony"] <= nousePosition!.y + 250 &&
          value!["nousePositiony"] >= nousePosition!.y - 250) {
        setState(() {
          i = i + 1;
        });
      }
      ...
      return 'Face id: ${leftEyePosition!.y},${rightEyePosition!
          .y},${bottommouthPosition!.y},${rightmouthPosition!
          .y},${leftmouthPosition!.y},${nousePosition!.y},';
    }).join('\n');
    getstaffdetails();
  ...
  enroll() async {
    ...
    final faceData2 = faces.map((face) async {
      ...
      final nouseLandmark = face.landmarks[FaceLandmarkType.noseBase];
      final nousePosition = nouseLandmark?.position;
      constants.firestore2db?.collection("Staffs").doc(staffid).collection(
          "Facedate").doc(staffid).set({
        ...
        children: [
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              child: Center(child: Text("OK", style: GoogleFonts.montserrat(
                  color: Colors.white,
        ...
        child: Column(
          ...
          children: [
            ...
            Padding(
              ...
              child: Text( 'Verifying your current location for school attendance. Please wait...',
                style: GoogleFonts.montserrat(
    ...
    Column(
      ...
      children: [
        ...
        SizedBox(
            // width: 350,
            width: width/1.02857,
            height: height/2.154285,
            child: Lottie.asset("assets/faceidd.json")),
...
/*                try {
                  await constants.firestore2db!.collection('SchoolLocation').add({
                    'lat': 13.1035585,
                    'long': 80.1808461,
                  });
             ...
             /*     Navigator.of(context).push(MaterialPageRoute(
              ...
              child: Container(
                ...
                child: Padding(
                  padding: EdgeInsets.only(top:height/50.26666, left: width/72, right:width/72),
                  child: Text(
                    textAlign: TextAlign.center,
                    "Mark Check IN",
                  // Text(constants.firestore2db.toString(),
                    style:
                  GoogleFonts.montserrat(color: Colors.white,
                      fontWeight: FontWeight.bold, fontSize: width / 26),),
                ),
              ),
            ),
            Column(
              ...
              /*  StreamBuilder(stream: constants.firestore2db!.collection('SchoolLocation').snapshots(), builder: (context, snapshot) {
                  if(snapshot.hasData){
                    return Text(snapshot.data!.docs[0]['long'].toString());
                  }else{
                    return SizedBox();
                  }
                },)*/
        ...
        SizedBox(height: height / 77.8,),
        GestureDetector(
          onTap: () {
            Navigator.push(context, MaterialPageRoute(builder: (context) =>
                Staff_View_Reports(widget.staffid, widget.schoolId),));
          },
          child: Container(
            height: height / 15.12,
            width: width / 1.125,
      ...
      children: [
        ...
        GestureDetector(
          ...
          child: Container(
            ...
            child: Center(child: Text("Enroll Face",
  ...
  showwwaring() {
    ...
    return AwesomeDialog(
        ...
        btnOkOnPress: () {
          Navigator.of(context).push(MaterialPageRoute(
              builder: (context) => DemoFaceid(widget.schoolId)));
        },
        btnOkText: "Continue"
    )
      ..show();
...
/*  Future<void> _getCurrentLocation() async {
    try {
      ...
      // Check if the current location is within 20 meters of any stored location in Firebase
      await checkLocationInRange(position.latitude, position.longitude);
    } catch (e) {
      print(e);
    }
  }
  Future<void> checkLocationInRange(double currentLat, double currentLng) async {
    // print('Latitude: $currentLat, Longitude: $currentLng');
    print('starts');
    try {
      QuerySnapshot querySnapshot = await constants.firestore2db!.collection('SchoolLocation').get();
      if (querySnapshot.docs.isNotEmpty) {
        for (QueryDocumentSnapshot doc in querySnapshot.docs) {
          String docId = doc.id;
          double lat = doc['lat'];
          double lng = doc['long'];
          double distance = Geolocator.distanceBetween(currentLat, currentLng, lat, lng);
          print('Distance to location with document ID $docId: $distance meters');
          if (distance <= 20) {
            print('Location with document ID $docId is within 20 meters.');
            return;
          }
        }
        print('No nearby location found within 20 meters.');
      } else {
        print('No locations found in Firestore');
      }
    } catch (e) {
  ...
  Future<void> _getCurrentLocation() async {
    try {
      ...
      // Print current location latitude and longitude
      ...
      if (querySnapshot.docs.isNotEmpty){
        for (QueryDocumentSnapshot doc in querySnapshot.docs) {
          ...
          if (distance <= 20) {
            ...
            print('Location with document ID $docId is NOT within 20 meters.');
            print('Red Zone');
            longDistance();
            // longDistance();
          }
        ...
        print('No locations found in Fire store');
      }
    } catch (e) {
      print('Error fetching locations: $e');
    }
