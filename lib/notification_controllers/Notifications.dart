...
class Notifications extends StatefulWidget {
  String ?Userdocid;
  String schoolId;
   Notifications(this.Userdocid, this.schoolId);
...
class _NotificationsState extends State<Notifications> {
      ...
      body:SingleChildScrollView(
        ...
        child: Column(
          children: [
            ...
            StreamBuilder(
                stream:constants.firestore2db?.collection("Staffs").doc(widget.Userdocid).collection("Notification").orderBy("timestamp",descending: true).snapshots(),
                builder: (context, snapshot) {
                  ...
                  if(snapshot.data!.docs.length==0){
                    return
                    /*  Center(child: Text("Notification are empty", style: TextStyle(fontSize: 22),)

                        ,);*/

                      Padding(
                        padding: EdgeInsets.only(top: height/4.54),
                        child: Center(
                          child: Container(
                              width: width / 0.9,
                              height: height / 3.78,
                              child: Lottie.asset(
                                  "assets/No Notification.json")
                          ),
                        ),
                      );
                  }
  ...
  readstatus(docid){
    constants.firestore2db?.collection("Staffs").doc(widget.Userdocid).collection("Notification").doc(docid).update({
      "readstatus":true
    });
  }
